...
const Navbar = () => {
    ...
    return (
        <nav className="navbar">
            <Link className="homelink" to ="/"> <h1 id="logo"> {t("Let's review")} </h1> </Link>
            <input className="searchbox" placeholder="Search"/>
            <div className = "links">
                <Link className="link" to = "/movies"> {t("All Movies")} </Link>
                <Link className="link" to> {t("Sign in")} </Link>                
            </div>
            <Dropdown as={ButtonGroup} className="translate">
                    ...
                    <Dropdown.Menu className="menu">
                        <Dropdown.Item onClick={() => i18next.changeLanguage("zh")}>中文</Dropdown.Item>
